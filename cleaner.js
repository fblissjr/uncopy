...
(function () {
  ...
  function unwrapPostmark(url) {
    // Matches track.pstmrk.it and click.pstmrk.it
    // Path format: /3s/encoded-destination/tracking-segments...
    // or /2s/encoded-destination/...
    ...
    // First segment is version prefix like "3s", "2s"
    if (!/^\d+s$/.test(pathParts[0])) return null;

    // Second segment is the URL-encoded destination
    const encoded = pathParts[1];
    try {
      const decoded = decodeURIComponent(encoded);
      // Reassemble: the destination may have its own path in subsequent segments
      // but Postmark encodes the full host+path in one segment, with query params
      // sometimes in the encoded part itself
      if (decoded.match(/^[a-zA-Z0-9][\w.-]+\.[a-zA-Z]{2,}/)) {
        // It's a bare domain (no protocol), add https://
  ...
  function unwrapSendGrid(url) {
    ...
    // SendGrid encodes destination URL in path segments
    const pathParts = url.pathname.split('/').filter(Boolean);
  ...
  // Specific rules (safe for automatic/content script use)
  ...
  function unwrapRedirects(urlStr, aggressive) {
    ...
    for (let depth = 0; depth < MAX_UNWRAP_DEPTH; depth++) {
      ...
      // Try generic rules if aggressive mode and nothing matched
  ...
  // Expose on window for content script and popup
  window.UnCopy = {
    cleanUrl: cleanUrl,
